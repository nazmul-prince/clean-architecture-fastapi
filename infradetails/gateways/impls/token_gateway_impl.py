...
class TokenGatewayImpl(BaseTokenGateway):
    def __init__(self):
    ...
    def validate_token(self, required_roles: Optional[List[str]] = None,
                       required_permissions: Optional[List[str]] = None):
        async def _get_current_user_sub_and_check(token: str = Depends(self.oauth2_scheme)):
            token_data = await self.introspect_token(token)
